...
export class BotGuardService implements AsyncInitializable {
    ...
    private async setup() {
        ...
        this.initializationPromise = this.initBotguard();
    ...
    private async initBotguard() {
        ...
        if (this.bgChallenge) {
            ...
            const integrityTokenResponse = await InnertubeProxy.fetch(buildURL('GenerateIT', true), {
                method: 'POST',
                headers: {
                    'content-type': 'application/json+protobuf',
                    'x-goog-api-key': GOOG_API_KEY,
